...
list_files <- list.files("/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/deletion_only/intermediate_files/", pattern = ".glm.linear", full.names = T, recursive = F)
...
for (p in pheno_list) {
	...
	files <- mixedsort(list.files("/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/deletion_only/intermediate_files/", pattern = paste0("\\.", p, ".glm.linear"), full.names = T, recursive = F))
	...
	if (nrow(sig) >= 1) {
		...
		fwrite(sig, paste0("/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/deletion_only/gwas_results/SCA_results/significant_", p, "_del_after_region_definition.txt.gz"), col.names = T, row.names = T, quote = F, sep = "\t")
...
fwrite(sig_all, paste0("/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/deletion_only/gwas_results/SCA_results/00_significant_del_after_region_definition.txt.gz"), col.names = T, row.names = F, quote = F, sep = "\t")
...
n <- nrow(as.data.frame(fread("/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/deletion_only/gwas_results/SCA_results/00_significant_del_after_region_definition.txt.gz", header = T, select = c(1:3, 8:9, 13:15))))
...
if (round == 1) {
		new_sig <- as.data.frame(fread("/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/deletion_only/gwas_results/SCA_results/00_significant_del_after_region_definition.txt.gz", header = T, select = c(1:3, 8:9, 13:15)))
	} else {
		new_sig <- as.data.frame(fread(paste0("/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/deletion_only/intermediate_files/significant_associations/sig_assoc_del_SCA_round_after_region_definition", round-1, ".txt.gz"), header = T, select = c(1:3, 8:9, 13:15)))}
...
if (round == 1) {
		...
		sum_file <- as.data.frame(fread(paste0("/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/deletion_only/intermediate_files/summary/summary_del_SCA_round_after_region_definition", round-1, ".txt"), header = T))
		sum_file <- rbind(sum_file, new_sig)}

print(paste0("Total number of independent signals after round ",round,": ", nrow(sum_file)))
fwrite(sum_file, paste0("/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/deletion_only/intermediate_files/summary/summary_del_SCA_round_after_region_definition", round, ".txt"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
...
for(i in 1:nrow(probes)) {
	...
	# Write file
	fwrite(data.frame(rs), paste0("/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/deletion_only/intermediate_files/cnv_profiles/", rs), col.names = F, row.names = F, quote = F, sep = "\t")

	# Extract profile with PLINK
	input <- paste0("/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/CNV_GWAS/plink_input/deletion_only/ukb_cnv_chr22") # PLINK file set for the deletion-only model
	output <- paste0("/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/deletion_only/intermediate_files/cnv_profiles/", rs)
...
for (i in 2:ncol(pheno)) {
	...
	for (rs in rs_list$ID) {
		rs_profile <- as.data.frame(fread(paste0("/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/deletion_only/intermediate_files/cnv_profiles/", rs, "_profile"), header = T))
...
fwrite(pheno_cor, paste0("/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/deletion_only/intermediate_files/phenotypes/pheno_cor_round_after_region_definition", round, ".txt"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
...
print("Starting GWAS")
system(paste0("mkdir /scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/deletion_only/intermediate_files/associations/All/round_after_region_definition", round))

# Define variables
input <- paste0("/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/CNV_GWAS/plink_input/deletion_only/ukb_cnv_chr22") # PLINK binary file set for deletion-only model
pheno_file <- paste0("/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/deletion_only/intermediate_files/phenotypes/pheno_cor_round_after_region_definition", round, ".txt") # INT, covariate-corrected phenotype file generate above
...
output <- paste0("/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/deletion_only/intermediate_files/associations/All/round_after_region_definition", round,"/gwas_del_SCA_round_after_region_definition", round,"_chr22")
...
list_gwas <- list.files(paste0("/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/deletion_only/intermediate_files/associations/All/round_after_region_definition", round), pattern = ".glm.linear", full.names = T, recursive = F)
...
fwrite(sig_gwas, paste0("/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/deletion_only/intermediate_files/significant_associations/sig_assoc_del_SCA_round__after_region_definition", round, ".txt.gz"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
rm(f, df, temp_sig)
