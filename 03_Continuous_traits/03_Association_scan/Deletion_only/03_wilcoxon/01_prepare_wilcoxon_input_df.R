...
# Load CNV-GWAS del-only model results (full table with annotated frequencies)
...
##############################################################
######### Prepare data frame with probe_level data for deletion x CN (deletion will be considered NA)
...
# Duplication will be assigned as missing values (NA)
probe_level[,-1][probe_level[,-1] == 1] <- NA


fwrite(probe_level, "/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/deletion_only/probe_level_data/22q11_2_copynumber_status_del_only_model.txt", col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
...
fwrite(df, "/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/deletion_only/intermediate_files/significant_continuousTraits_probeLevel_raw.txt", col.names = T, row.names = F, quote = F, sep = "\t")
