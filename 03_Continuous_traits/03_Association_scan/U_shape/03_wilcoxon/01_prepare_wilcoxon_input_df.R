...
# Load CNV-GWAS u-shape model results 
...
# Save to use as input in array
fwrite(as.data.frame(traits_vector), "/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/u_shape/wilcoxon_test/input/list_traits.txt", quote = F, col.names= F, row.names = F, sep = "\t")

##############################################################
######### Prepare data frame with probe_level data for u-shape x CN (deletion will be considered NA)
...
# Deletions will be assigned as 1, so that they are in the same group as duplications
probe_level[,-1][probe_level[,-1] == -1] <- 1


fwrite(probe_level, "/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/u_shape/probe_level_data/22q11_2_copynumber_status_u_shape_model.txt", col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
...
# Load phenotype data (continuous, corrected) only for traits of interest (significant traits)
...
fwrite(df, "/scratch/beegfs/FAC/FBM/DBC/zkutalik/default_sensitive/mzamario/Traits_22q112_region_CNV_GWAS/Continuous/u_shape/intermediate_files/significant_continuousTraits_probeLevel_raw.txt", col.names = T, row.names = F, quote = F, sep = "\t")
